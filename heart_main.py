...
class HeartConfig(Config):
    ...
    STEPS_PER_EPOCH = 32#45

    # Number of validation steps to run at the end of every training epoch.
    VALIDATION_STEPS = 5#10
    ...
    # optimizer : sgd or adamw, adamw always wrong
    OPTIMIZER = "adamw"

    LEARNING_RATE = 0.0001   #adamw:0.0001  
    LEARNING_MOMENTUM = 0.9

    # Weight decay regularization
    WEIGHT_DECAY = 0.0005   #adamw:0.0005

    #edge loss(bool) adamw can't add edgeloss, sgd can.
    EDGE_LOSS = False

    RPN_BBOX_LOSS = "ciou_loss" #"smooth_l1_loss" #"ciou_loss" #"iou_loss"
    MRCNN_BBOX_LOSS = "ciou_loss" #"smooth_l1_loss" #"ciou_loss" #"iou_loss"  #try ciou_loss, default is smooth
    ...
    MASK_POOL_SIZE = [96,96,96]     ###[96, 96, 96] for train, [64,64,64] for ablation
    ...
    # LOSS_WEIGHTS = {
    #     "rpn_class_loss": 1.,  # correct detection in rpn is proved to be very important!
    #     "rpn_bbox_loss": 1.,
    #     "mrcnn_class_loss": 1.,
    #     "mrcnn_bbox_loss": 1.,
    #     "mrcnn_mask_loss": 1.,
...
class HeartDataset(utils.Dataset):
    ...
    def load_heart(self, subset):
        ...
        for a in info:
            image = nib.load(a['path']).get_data().copy()
...
def test(model, limit, save, bbox):
    ...
    detect_time = 0
    load_detect_time = 0
    load_detect_ious_time = 0
    for path in info[:limit]:
        load_start_time = time.time()   #load + detect total time starter
        ...
        load_detect_time += time.time() - load_start_time
        ...
        mask = result["mask"]
        mask_key = [205, 420, 500, 550, 600, 820, 850]
        # Prepare the gt-masks and pred-masks to calculate the ious.
        gt_masks = np.zeros((image.shape[0], image.shape[1], image.shape[2], model.config.NUM_CLASSES - 1),dtype=np.float32)
        pred_masks = np.zeros((image.shape[0], image.shape[1], image.shape[2], model.config.NUM_CLASSES - 1),dtype=np.float32)
        # Generate the per instance gt masks.
        for j in range(model.config.NUM_CLASSES - 1):
            # gt_masks[:, :, :, j][label == mask_key[j]] = 1
            gt_masks[:, :, :, j][label == j+1] = 1
        # Generate the per instance predicted masks.
        for j in range(model.config.NUM_CLASSES - 1):
            pred_masks[:, :, :, j][mask == j + 1] = 1
        # calculate different kind of ious, dice, hd
        per_class_iou = utils.compute_per_class_mask_iou(gt_masks, pred_masks)
        per_class_ious.append(per_class_iou)
        per_class_dice = utils.compute_per_class_mask_dice(gt_masks, pred_masks)
        per_class_dices.append(per_class_dice)#(2*per_class_iou)/(1+per_class_iou))
        #per_class_hd

        load_detect_ious_time += time.time() - load_start_time
        ...
        wholeMaskIou = utils.compute_mask_iou(label, mask)

    print("Test completed.")
    # Print the iou results.
    per_class_ious = np.array(per_class_ious)
    per_class_dices = np.array(per_class_dices)
    print("per class iou mean:", np.mean(per_class_ious, axis=0))
    print("iou std:", np.std(per_class_ious, axis=0))
    print("per class dice mean:", np.mean(per_class_dices, axis = 0))
    print("dice std:", np.std(per_class_dices, axis=0))

    print("Total ious mean:", per_class_ious.mean())
    print("Total dice mean:", per_class_dices.mean())
    print("Total ious std mean:", np.std(per_class_ious, axis=0).mean())
    print("Total dice std mean:", np.std(per_class_dices, axis=0).mean())
    # print time consumption
    print("Total detect time:", detect_time)
    print("Total load+detect time:", load_detect_time)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--command",default="train",
                        metavar="<command>",
                        help="'train' or 'test'")
    parser.add_argument('--weights', required=False, default="None",    #True
                        metavar="/path/to/weights.pth",
                        help="Path to weights .pth file")
    parser.add_argument('--stage', required=False, default="beginning", #True
