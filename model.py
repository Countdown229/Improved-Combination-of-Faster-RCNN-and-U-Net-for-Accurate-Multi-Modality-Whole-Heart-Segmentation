...
class FPN(nn.Module):
    def __init__(self, C1, C2, C3, out_channels, config):
        ...
        self.C2 = C2
        self.C3 = C3
        self.P3_conv1 = nn.Conv3d(config.BACKBONE_CHANNELS[1] * 4, self.out_channels, kernel_size=1, stride=1)  #128,128, 1
        self.P3_conv2 = nn.Conv3d(self.out_channels, self.out_channels, kernel_size=3, stride=1, padding=1)     #128,128, 3
        self.P2_conv1 = nn.Conv3d(config.BACKBONE_CHANNELS[0] * 4, self.out_channels, kernel_size=1, stride=1)  #64, 128, 1 
        self.P2_conv2 = nn.Conv3d(self.out_channels, self.out_channels, kernel_size=3, stride=1, padding=1)     #128,128, 3

    def forward(self, x):
        x = self.C1(x)  #[1,16,48,80,80]
        x = self.C2(x)  
        c2_out = x      #[1,64,24,40,40]
        x = self.C3(x)  
        c3_out = x      #[1,128,12,20,20]

        p3_out = self.P3_conv1(c3_out)  #[1,128,12,20,20]
        p2_out = self.P2_conv1(c2_out) + F.upsample(p3_out, scale_factor=2)#[1,128,24,40,40]
        p3_out = self.P3_conv2(p3_out)  #[1,128,12,20,20]
        p2_out = self.P2_conv2(p2_out)  #[1,128,24,40,40]
...
def apply_box_deltas(boxes, deltas):
    ...
    # Apply deltas
    center_z = center_z + deltas[:, 0] * depth
    center_y = center_y + deltas[:, 1] * height
    center_x = center_x + deltas[:, 2] * width
    depth = depth * torch.exp(deltas[:, 3])
    height = height * torch.exp(deltas[:, 4])
    width = width * torch.exp(deltas[:, 5])
...
def pyramid_roi_align(inputs, pool_size, test_flag=False):
    ...
    [12,12,12] for classifier       [96,96,96] for mask
    ...
    roi_level = roi_level.clamp(2, 3)   #[12,1]
    ...
    for i, level in enumerate(range(2, 4)):
        ...
        pooled_features = RoI_Align(feature_maps[i], pool_size, level_boxes)    #[2,128,12,12,12][10,128,12,12,12]  [4,1,96,96,96]
        pooled.append(pooled_features)

    # Pack pooled features into one tensor
    pooled = torch.cat(pooled, dim=0)   #[12,128,12,12,12]  [4,1,96,96,96]

    # Pack box_to_level mapping into one array and add another
    # column representing the order of pooled boxes
    box_to_level = torch.cat(box_to_level, dim=0)   #[12]   [4]

    # Rearrange pooled features to match the order of the original boxes
    _, box_to_level = torch.sort(box_to_level)  #sorted index
    pooled = pooled[box_to_level, :, :, :]  #[12,128,12,12,12]  [4,1,96,96,96]->[4,128,96,96,96]
...
class RPN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv_shared(x))  #【1,128,24,40,40】-> [1,256,24,40,40]

        # Anchor Score. [batch, anchors per location * 2, depth, height, width].
        rpn_class_logits = self.conv_class(x)   #【1,2,24,40,40】
        ...
        return [rpn_class_logits, rpn_probs, rpn_bbox, x]
...
class Classifier(nn.Module):
    def __init__(self, channel, pool_size, image_shape, num_classes, fc_size, test_flag=False):# 128; [12,12,12]; 128; 2; 128
    ...
    def forward(self, x, rois):
        x = pyramid_roi_align([rois] + x, self.pool_size, self.test_flag)   #[3,128,12,12,12]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
...
def ciou_loss(pred, target, eps=1e-7):
    """
    Args:
        pred (Tensor): Predicted bboxes of format (z1, y1, x1, z2, y2, x2),
            shape (n, 6).
        target (Tensor): Corresponding gt bboxes, shape (n, 6).
        eps (float): Eps to avoid log(0).
    Return:
        Tensor: Loss tensor.
    """
    # overlap
    lt = torch.max(pred[:, :3], target[:, :3])
    rb = torch.min(pred[:, 3:], target[:, 3:])
    dhw = (rb - lt).clamp(min=0)
    overlap = dhw[:, 0] * dhw[:, 1] * dhw[:, 2]

    # union
    ap = (pred[:, 3] - pred[:, 0]) * (pred[:, 4] - pred[:, 1]) * (pred[:, 5] - pred[:, 2])
    ag = (target[:, 3] - target[:, 0]) * (target[:, 4] - target[:, 1]) * (target[:, 5] - target[:, 2])
    ...
    # enclose area
    enclose_z1y1x1 = torch.min(pred[:, :3], target[:, :3])
    enclose_z2y2x2 = torch.max(pred[:, 3:], target[:, 3:])
    enclose_dhw = (enclose_z2y2x2 - enclose_z1y1x1).clamp(min=0)

    cd = enclose_dhw[:, 0]
    ch = enclose_dhw[:, 1]
    cw = enclose_dhw[:, 2]

    c2 = cd**2 + ch**2 + cw**2 + eps

    b1_z1, b1_y1, b1_x1 = pred[:, 0], pred[:, 1], pred[:, 2]
    b1_z2, b1_y2, b1_x2 = pred[:, 3], pred[:, 4], pred[:, 5]
    b2_z1, b2_y1, b2_x1 = target[:, 0], target[:, 1], target[:, 2]
    b2_z2, b2_y2, b2_x2 = target[:, 3], target[:, 4], target[:, 5]
    
    depth = ((b2_z1 + b2_z2) - (b1_z1 + b1_z2))**2 / 4
    left  = ((b2_x1 + b2_x2) - (b1_x1 + b1_x2))**2 / 4
    right = ((b2_y1 + b2_y2) - (b1_y1 + b1_y2))**2 / 4
    rho2 = depth + left + right
    
    d1, h1, w1 = b1_z2 - b1_z1, b1_y2 - b1_y1, b1_x2 - b1_x1 + eps
    d2, h2, w2 = b2_z2 - b2_z1, b2_y2 - b2_y1, b2_x2 - b2_x1 + eps

    # 对应公式
    factor = 2 / math.pi**2
    v = factor * (torch.pow(torch.atan(d2 / h2) - torch.atan(d1 / h1), 2) + torch.pow(torch.atan(d2 / w2) - torch.atan(d1 / w1), 2))
...
def compute_rpn_bbox_loss(target_bbox, rpn_match, rpn_bbox, anchors, config):
    ...
    # Smooth L1 loss
    if config.RPN_BBOX_LOSS =="smooth_l1_loss":
        loss = F.smooth_l1_loss(rpn_bbox, target_bbox)
    # ciou loss
    elif config.RPN_BBOX_LOSS == "ciou_loss" or config.RPN_BBOX_LOSS == "iou_loss":
        std_dev = torch.from_numpy(np.reshape(config.RPN_BBOX_STD_DEV, [1, 6])).float()
        if config.GPU_COUNT:
            std_dev = std_dev.cuda()
        b = target_bbox * std_dev
        c = rpn_bbox * std_dev
        a = anchors[indices.detach()[:, 1]]
        a_d = a[:, 3] - a[:, 0]
        a_h = a[:, 4] - a[:, 1]
        a_w = a[:, 5] - a[:, 2]
        a_c_z = a[:, 0] + 0.5*a_d
        ...
        b_d = torch.exp(b[:, 3]) * a_d
        b_h = torch.exp(b[:, 4]) * a_h
        b_w = torch.exp(b[:, 5]) * a_w
        c_d = torch.exp(c[:, 3]) * a_d
        c_h = torch.exp(c[:, 4]) * a_h
        c_w = torch.exp(c[:, 5]) * a_w
        
        gt_bbox = torch.from_numpy(np.zeros((a.shape[0], 6))).float().cuda()
        pred_bbox = torch.from_numpy(np.zeros((a.shape[0], 6))).float().cuda()
        
        gt_bbox[:, 0] = b_c_z - 0.5 * b_d
        gt_bbox[:, 3] = b_c_z + 0.5 * b_d
        gt_bbox[:, 1] = b_c_y - 0.5 * b_h
        gt_bbox[:, 4] = b_c_y + 0.5 * b_h
        gt_bbox[:, 2] = b_c_x - 0.5 * b_w
        gt_bbox[:, 5] = b_c_x + 0.5 * b_w
        pred_bbox[:, 0] = c_c_z - 0.5 * c_d
        pred_bbox[:, 3] = c_c_z + 0.5 * c_d
        pred_bbox[:, 1] = c_c_y - 0.5 * c_h
        pred_bbox[:, 4] = c_c_y + 0.5 * c_h
        pred_bbox[:, 2] = c_c_x - 0.5 * c_w
        pred_bbox[:, 5] = c_c_x + 0.5 * c_w

        if config.RPN_BBOX_LOSS == "ciou_loss":
            loss = torch.mean(ciou_loss(pred_bbox, gt_bbox))
        elif config.RPN_BBOX_LOSS == "iou_loss":
            loss = torch.mean(iou_loss(pred_bbox, gt_bbox))
...
def compute_mrcnn_bbox_loss(p_rois, target_bbox, target_class_ids, pred_bbox, config):
    ...
    if target_class_ids.size()[0] != 0:
        ...
        if config.MRCNN_BBOX_LOSS == "smooth_l1_loss":
        # Smooth L1 loss
            loss = F.smooth_l1_loss(pred_bbox, target_bbox)
        elif (config.MRCNN_BBOX_LOSS == "ciou_loss" or config.MRCNN_BBOX_LOSS == "iou_loss") and p_rois.size()[0] > 0:
            std_dev = torch.from_numpy(config.BBOX_STD_DEV).float()
            if config.GPU_COUNT:
                std_dev = std_dev.cuda()           #[6]
            bb = target_bbox * std_dev   #[1,6]
            cc = pred_bbox * std_dev     #[1,6]
            aa = p_rois  #[1,6]
            aa_d = aa[:, 3] - aa[:, 0]
            aa_h = aa[:, 4] - aa[:, 1]
            aa_w = aa[:, 5] - aa[:, 2]
            aa_c_z = aa[:, 0] + 0.5*aa_d
            aa_c_y = aa[:, 1] + 0.5*aa_h
            aa_c_x = aa[:, 2] + 0.5*aa_w

            bb_c_z = bb[:, 0] * aa_d + aa_c_z
            bb_c_y = bb[:, 1] * aa_h + aa_c_y
            bb_c_x = bb[:, 2] * aa_w + aa_c_x
            cc_c_z = cc[:, 0] * aa_d + aa_c_z
            cc_c_y = cc[:, 1] * aa_h + aa_c_y
            cc_c_x = cc[:, 2] * aa_w + aa_c_x

            bb_d = torch.exp(bb[:, 3]) * aa_d
            bb_h = torch.exp(bb[:, 4]) * aa_h
            bb_w = torch.exp(bb[:, 5]) * aa_w
            cc_d = torch.exp(cc[:, 3]) * aa_d
            cc_h = torch.exp(cc[:, 4]) * aa_h
            cc_w = torch.exp(cc[:, 5]) * aa_w
            
            ggt_bbox = torch.from_numpy(np.zeros((aa.shape[0], 6))).float().cuda()
            ppred_bbox = torch.from_numpy(np.zeros((aa.shape[0], 6))).float().cuda()  #[1,6]
            
            ggt_bbox[:, 0] = bb_c_z - 0.5 * bb_d
            ggt_bbox[:, 3] = bb_c_z + 0.5 * bb_d
            ggt_bbox[:, 1] = bb_c_y - 0.5 * bb_h
            ggt_bbox[:, 4] = bb_c_y + 0.5 * bb_h
            ggt_bbox[:, 2] = bb_c_x - 0.5 * bb_w
            ggt_bbox[:, 5] = bb_c_x + 0.5 * bb_w
            ppred_bbox[:, 0] = cc_c_z - 0.5 * cc_d
            ppred_bbox[:, 3] = cc_c_z + 0.5 * cc_d
            ppred_bbox[:, 1] = cc_c_y - 0.5 * cc_h
            ppred_bbox[:, 4] = cc_c_y + 0.5 * cc_h
            ppred_bbox[:, 2] = cc_c_x - 0.5 * cc_w
            ppred_bbox[:, 5] = cc_c_x + 0.5 * cc_w

            if config.MRCNN_BBOX_LOSS == "ciou_loss":
                loss = torch.mean(ciou_loss(ppred_bbox, ggt_bbox))    #tensor() []
            elif config.MRCNN_BBOX_LOSS == "iou_loss":
                loss = torch.mean(iou_loss(ppred_bbox, ggt_bbox)) 
            print("MRCNN ENTER IF")
        else:
            loss = Variable(torch.FloatTensor([0]), requires_grad=False)
            if target_class_ids.is_cuda:
                loss = loss.cuda()
    ...
    if target_class_ids.size()[0] != 0:
        ...
        for i in range(indices.size()[0]):
            ...
            for j in range(7):
                ...
                # Mean Square Error
                loss = loss + loss_fn(y_pred_final, y_true_final)
        loss = loss / indices.size()[0]
    else:
        loss = Variable(torch.FloatTensor([0]), requires_grad=False)
        if target_class_ids.is_cuda:
            loss = loss.cuda()
...
def compute_losses(rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits,
                   p_rois, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, mrcnn_mask_logits, config, anchors):

    rpn_class_loss = compute_rpn_class_loss(rpn_match, rpn_class_logits)    #[1,43200,1] [1,43200,2] 
    rpn_bbox_loss = compute_rpn_bbox_loss(rpn_bbox, rpn_match, rpn_pred_bbox, anchors, config)  #[1,128,6] [1,43200,1] [1,43200,6] [43200,6] []
    mrcnn_class_loss = compute_mrcnn_class_loss(torch.from_numpy(np.where(target_class_ids.cpu() > 0, 1, 0)).cuda(),
                                                mrcnn_class_logits)
    mrcnn_bbox_loss = compute_mrcnn_bbox_loss(p_rois, target_deltas,
                                              torch.from_numpy(np.where(target_class_ids.cpu() > 0, 1, 0)).cuda(), mrcnn_bbox, config)

    mrcnn_mask_loss = compute_mrcnn_mask_loss(target_mask, target_class_ids, mrcnn_mask_logits)
    
    #edge loss
    if config.EDGE_LOSS == True:
...
def build_rpn_targets(anchors, gt_boxes, config):
    ...
    for i, a in zip(ids, anchors[ids]):
        ...
        # Normalize
        rpn_bbox[ix] = rpn_bbox[ix] / config.RPN_BBOX_STD_DEV
        ix = ix + 1
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config, test_flag=False):
        ...
        h, w, d = config.IMAGE_SHAPE[:3]    #[320,320,192]
        ...
        self.optimizer = optim.AdamW([{'params': trainables_wo_bn, 'weight_decay': self.config.WEIGHT_DECAY},             \
                                 {'params': trainables_only_bn, 'weight_decay': 0}], lr=self.config.LEARNING_RATE)   \
                    if self.config.OPTIMIZER == "adamw" else                                                         \
                    optim.SGD([{'params': trainables_wo_bn, 'weight_decay': self.config.WEIGHT_DECAY},               \
                               {'params': trainables_only_bn}], lr=self.config.LEARNING_RATE, momentum=self.config.LEARNING_MOMENTUM)
    ...
    def predict(self, inputs, mode):
        molded_images = inputs[0]   #[1,1,192,320,320]
        ...
        # Feature extraction
        p2_out, p3_out = self.fpn(molded_images)    #[1,128,24,40,40] [1,128,12,20,20]
        ...
        #mrcnn_mask_feature_maps = []
        # Loop through pyramid layers
        layer_outputs = []  # list of lists
        for p in rpn_feature_maps:
            #pp = self.rpn(p)
            #layer_outputs.append(pp[:3])   #
            layer_outputs.append(self.rpn(p)[:3])   #[1,38400,2], [1,38400,2], [1,38400,6]
            #mrcnn_mask_feature_maps.append(pp[-1])
            #del pp
        ...
        if mode == 'inference':
            ...
            if rois.size()[0] == 0:
                ...
                # Create masks for detections
                mrcnn_mask_logits, mrcnn_mask = self.mask(mrcnn_mask_feature_maps, p_rois)  #p_rois:[4,6]

        return [rpn_class_logits, rpn_bbox,
                    target_class_ids, mrcnn_class_logits,
                    p_rois, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, mrcnn_mask_logits]
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs):
        ...
        val_generator = torch.utils.data.DataLoader(val_set, batch_size=1, shuffle=True, num_workers=2)
        ...
        for epoch in range(self.epoch + 1, epochs + 1):
            ...
            loss, loss_rpn_class, loss_rpn_bbox, \
                ...
                self.train_epoch(train_generator, self.config.STEPS_PER_EPOCH, angle, train_dataset)
    ...
    def train_epoch(self, datagenerator, steps, angle, dataset):
        batch_count = 0
        loss_sum = 0
        ...
        for inputs in datagenerator:
            ...
            rpn_class_logits, rpn_pred_bbox, target_class_ids, \
                mrcnn_class_logits, p_rois, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, mrcnn_mask_logits = \
            ...
            rpn_class_loss, rpn_bbox_loss, \
                ...
                compute_losses(rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_class_ids,
                               mrcnn_class_logits, p_rois, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, mrcnn_mask_logits, self.config, self.anchors)
            ...
            # Statistics
            loss_sum = loss_sum + loss.detach().cpu().item() / steps
            loss_rpn_class_sum = loss_rpn_class_sum + rpn_class_loss.detach().cpu().item() / steps
            loss_rpn_bbox_sum = loss_rpn_bbox_sum + rpn_bbox_loss.detach().cpu().item() / steps
            loss_mrcnn_class_sum = loss_mrcnn_class_sum + mrcnn_class_loss.detach().cpu().item() / steps
            loss_mrcnn_bbox_sum = loss_mrcnn_bbox_sum + mrcnn_bbox_loss.detach().cpu().item() / steps
            loss_mrcnn_mask_sum = loss_mrcnn_mask_sum + mrcnn_mask_loss.detach().cpu().item() / steps
            loss_mrcnn_mask_edge_sum = loss_mrcnn_mask_edge_sum + mrcnn_mask_edge_loss.detach().cpu().item() / steps
